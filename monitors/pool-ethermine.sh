...
if [ "$API_STATUS" != "OK" ]; then
	...
	WORKER_TAG_FIELDS_AND_TIME=`echo $WORKERS_OUTPUT | jq -r '.data[] | "rig_id=\(.worker) reported_hr=\(.reportedHashrate),current_hr=\(.currentHashrate),valid_shares=\(.validShares),invalid_shares=\(.invalidShares),stale_shares=\(.staleShares),avg_hr_24h=\(.averageHashrate) \(.time)000000000"' `
