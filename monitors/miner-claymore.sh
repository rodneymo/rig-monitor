...
if [ "$CLAYMORE_READOUT" == "" ]; then
        echo "w3m FAILED"
        LINE="miner_system,rig_id=${RIG_ID},miner=claymore,coin=${COIN_LABEL},dcoin=${DCOIN_LABEL} installed_gpus=${INSTALLED_GPUS},active_gpus=-1,target_hr=${TARGET_HR},hr=-1,hr_dcoin=-1,target_hr_dcoin=${TARGET_HR_DCOIN}"
	...
	DATA_POINTS=`awk -f ${BASE_DIR}/awk/parse_claymore_status.awk \
		...
		target_hr=${TARGET_HR} target_hr_dcoin=${TARGET_HR_DCOIN} \
		<<< "$CLAYMORE_READOUT" `
